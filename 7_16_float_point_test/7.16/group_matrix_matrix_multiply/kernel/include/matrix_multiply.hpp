                                ...
                                uint32_t A_HEIGHT, uint32_t A_WIDTH, uint32_t B_WIDTH)
{
  ...
  float A_local[num_row_A_per_thread * A_WIDTH];
  float B_local[A_WIDTH * B_WIDTH];
                                ...
                                uint32_t A_HEIGHT, uint32_t A_WIDTH, uint32_t B_WIDTH)
{
  ...
  float A_local[A_HEIGHT * A_WIDTH];
  float B_local[num_col_B_per_thread * B_WIDTH];
  // copy data
  memcpy (A_local, A, sizeof(A[0])*A_HEIGHT*A_WIDTH);
...
template <typename TA, typename TB, typename TC>
int  __attribute__ ((noinline)) matrix_multiply_group_v18(TA *A, TB *B, TC *C, // matrix multiplication for v3, v3 implement the data distribution along column side, without scratchpad
                                uint32_t A_HEIGHT, uint32_t A_WIDTH, uint32_t B_HEIGHT, uint32_t B_WIDTH)
{
  ...
  int A_num_blk_per_row = (A_WIDTH) / block_size;
  int A_num_blk_per_col = (A_HEIGHT) / block_size;
  int B_num_blk_per_row = (B_HEIGHT) / block_size;
  int B_num_blk_per_col = (B_WIDTH) / block_size;
  ...
  for (int A_blk_row_id = 0; A_blk_row_id < A_num_blk_per_col; A_blk_row_id ++){
        for (int B_blk_col_id = tid; B_blk_col_id < B_num_blk_per_col; B_blk_col_id += num_threads){
              ...
              for (int local_C_row = 0; local_C_row < block_size; local_C_row ++){
                    for (int local_C_col = 0; local_C_col < block_size; local_C_col ++){
                          C_local[local_C_row * block_size + local_C_col] = 0;
                    }
              }
              ...
              for (int inside_blk_id = 0; inside_blk_id < A_num_blk_per_row; inside_blk_id ++){
                    ...
                    for (int local_A_row = A_blk_row_id * block_size; local_A_row < (A_blk_row_id + 1) * block_size; local_A_row ++){
                          memcpy(&A_local[local_cpy_A_idx * block_size], &A[local_A_row * A_WIDTH + local_A_col_start], sizeof(A[0]) * block_size);
                          local_cpy_A_idx ++;
                    }

                    //copy matrix B block into the B_local scratchpad
                    local_B_col_start = inside_blk_id * block_size;
                    for (int local_B_row = B_blk_col_id * block_size; local_B_row < (B_blk_col_id + 1) * block_size; local_B_row ++){
                          memcpy(&B_local[local_cpy_B_idx * block_size], &B[local_B_row * B_HEIGHT + local_B_col_start], sizeof(B[0]) * block_size);
                          local_cpy_B_idx ++;
                    }
              ...
              for (int memory_row_id = A_blk_row_id * block_size; memory_row_id < (A_blk_row_id + 1) * block_size; memory_row_id ++){
                  memback_col_idx = 0;
                  for (int memory_col_id = B_blk_col_id * block_size; memory_col_id < (B_blk_col_id + 1) * block_size; memory_col_id ++){
                        C[memory_row_id * B_WIDTH + memory_col_id] = C_local[memback_row_idx * block_size + memback_col_idx];
                        memback_col_idx ++;
                  }
                  memback_row_idx ++;
