#ifndef __V20_FUNCTION_HPP
#define __V20_FUNCTION_HPP
#include <cstdint>
#include <cstring>
...
template <typename TA, typename TB, typename TC>
int  __attribute__ ((noinline)) matrix_multiply_group_v20(TA *A, TB *B, TC *C, // matrix multiplication for v3, v3 implement the data distribution along column side, without scratchpad
                                uint32_t A_HEIGHT, uint32_t A_WIDTH, uint32_t B_HEIGHT, uint32_t B_WIDTH,
                                uint32_t block_size_row, uint32_t block_size_col)
{
