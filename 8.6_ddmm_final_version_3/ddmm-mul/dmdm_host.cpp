...
int kernel_matrix_matrix_multiply (int argc, char **argv) {
    ...
    if(!strcmp("v0", test_name)){        // v0 (1 x 4)implement the data distribution along row side, with scratchpad
        ...
        grid_dim = { .x = 1, .y = 1};       // block size could be flexible, could also deal with the condition when the matrix size could not be divided by the block size, with unrolling method and outer product
        tg_dim = { .x = 4, .y = 4 };        // old parameterize the number of registers, (15 < num_register < 20)
    }else {
