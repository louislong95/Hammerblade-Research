                                ...
                                uint32_t A_HEIGHT, uint32_t A_WIDTH, uint32_t B_WIDTH)
{
  ...
  int row_A_end = (((tid_y + 1) * block_size > A_HEIGHT)) ? A_HEIGHT : ((tid_y + 1) * block_size);
  int col_A_start = tid_x * block_size;
  int col_A_end = (((tid_x + 1) * block_size > A_WIDTH)) ? A_WIDTH : ((tid_x + 1) * block_size);
  ...
  int row_B_end = (tid_x == 0) ? block_size : (2 * block_size);
  int col_B_start = (tid_x == 0) ? 0 : block_size;
  int col_B_end = (tid_x == 0) ? block_size : (2 * block_size);
...
// copy matrix A block into scratchpad-stage 2
  ...
  row_A_end = (((tid_y + 1) * block_size > A_HEIGHT)) ? A_HEIGHT : ((tid_y + 1) * block_size);
  ...
  row_B_end = (tid_x == 0) ? block_size : (2 * block_size);
  col_B_start = (tid_x == 0) ? block_size : 0;
  col_B_end = (tid_x == 0) ? (2 * block_size) : block_size;
