...
extern "C" {
        int  __attribute__ ((noinline)) kernel_matrix_multiply_float(
                ...
                if(extra_blk_row == 0){
                        ...
                        if(block_size_row != 1){
                                if((block_size_row == self_blk_row_per_tile) && (block_size_row < 11)){
                ...
                if ( A_WIDTH < 2){              // if the matrix size is not so large, select the row by row multiplication
                        ...
                        rc = matrix_multiply_group_v27(A, B, C,
                                A_HEIGHT, A_WIDTH, B_HEIGHT, B_WIDTH, block_row);
                }
