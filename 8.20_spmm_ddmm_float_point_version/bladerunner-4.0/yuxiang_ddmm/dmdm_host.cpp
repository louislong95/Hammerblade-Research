...
#define NUM_REGISTER 20 // the width of the block (only 4, 8, 12, 16, 20))
...
int run_test(hb_mc_device_t &device, const char* kernel,
    ...
    uint32_t cuda_argv[7] = {A_device, B_device, C_device,
        A_HEIGHT, A_WIDTH, B_WIDTH, NUM_REGISTER
        };
...
int kernel_matrix_matrix_multiply (int argc, char **argv) {
    ...
    if(!strcmp("v0", test_name)){        // v0 (1 x 4)implement the data distribution along row side, with scratchpad
        ...
        grid_dim = { .x = 1, .y = 1};
        tg_dim = { .x = 4, .y = 4 };
    }else if(!strcmp("v19", test_name)){  // v19 (4 x 4)implement the dd matrix mulitplication with blocking strategy, using scratchpad to store the matrix blocks, matrix B transposing,suitable for non-square block size
        grid_dim = { .x = 1, .y = 1};
        tg_dim = { .x = 4, .y = 4 };
    }else if(!strcmp("v20", test_name)){  // v20 (4 x 4)implement the dd matrix mulitplication with blocking strategy, using scratchpad to store the matrix blocks, matrix B transposing,suitable for all matrix size
        grid_dim = { .x = 1, .y = 1};       // could switch between row by row multiplication and blocking strategy based on the matrix size
        tg_dim = { .x = 4, .y = 4 };
    }else if(!strcmp("v21", test_name)){  // v21 (4 x 4)implement the dd matrix mulitplication with improved blocking strategy, using scratchpad to store the matrix blocks, matrix B transposing,suitable for all matrix size
        grid_dim = { .x = 1, .y = 1};       // block size must be square, but could also deal with the condition when the matrix size could not be divided by the block size
        tg_dim = { .x = 4, .y = 4 };
    }else if(!strcmp("v22", test_name)){  // v22 (4 x 4)implement the dd matrix mulitplication with improved blocking strategy, using scratchpad to store the matrix blocks, matrix B transposing,suitable for all matrix size
        grid_dim = { .x = 1, .y = 1};       // block size could be flexible, could also deal with the condition when the matrix size could not be divided by the block size
        tg_dim = { .x = 4, .y = 4 };       
    }else if(!strcmp("v23", test_name)){  // v23 (4 x 4)implement the dd matrix mulitplication with improved blocking strategy, using scratchpad to store the matrix blocks, matrix B transposing,suitable for all matrix size
        grid_dim = { .x = 1, .y = 1};       // block size could be flexible, could also deal with the condition when the matrix size could not be divided by the block size, with unrolling method
        tg_dim = { .x = 4, .y = 4 };        // register number fixed at 20
    }else if(!strcmp("v24", test_name)){  // v24 (4 x 4)implement the dd matrix mulitplication with improved blocking strategy, using scratchpad to store the matrix blocks, matrix B transposing, suitable for all matrix size
        grid_dim = { .x = 1, .y = 1};       // block size could be flexible, could also deal with the condition when the matrix size could not be divided by the block size, with unrolling method
        tg_dim = { .x = 4, .y = 4 };        // old parameterize the number of registers, (0 < num_register < 20)
    }else if(!strcmp("v25", test_name)){  // v25 (4 x 4)implement the dd matrix mulitplication with improved blocking strategy, using scratchpad to store the matrix blocks, matrix B transposing, suitable for all matrix size
        grid_dim = { .x = 1, .y = 1};       // block size is flexible, could also deal with the condition when the matrix size could not be divided by the block size, with unrolling method and outer product(in matrix block)
        tg_dim = { .x = 4, .y = 4 };        // register number fixed at 20, make use of function pointer
    }else if(!strcmp("v26", test_name)){  // v26 (4 x 4)implement the dd matrix mulitplication with improved blocking strategy, using scratchpad to store the matrix blocks, matrix B transposing, suitable for all matrix size
        grid_dim = { .x = 1, .y = 1};       // block size is flexible, could also deal with the condition when the matrix size could not be divided by the block size, with unrolling method and outer product(in matrix block)
        tg_dim = { .x = 4, .y = 4 };        // register number fixed at 20, not make use of function pointer
    }else if(!strcmp("v27", test_name)){  // v27 (4 x 4)implement the dd matrix mulitplication with improved blocking strategy, using scratchpad to store the matrix blocks, matrix B transposing, suitable for all matrix size
        grid_dim = { .x = 1, .y = 1};       // block size is flexible, could also deal with the condition when the matrix size could not be divided by the block size, with unrolling method and outer product(whole matrix)
        tg_dim = { .x = 4, .y = 4 };        // register number fixed at 20, not make use of function pointer, distribute rows of matrix A to tiles
    }else {
