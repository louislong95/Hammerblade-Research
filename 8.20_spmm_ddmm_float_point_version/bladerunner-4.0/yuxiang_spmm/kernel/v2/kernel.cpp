...
int  __attribute__ ((noinline)) kernel_sm_dm(sparse_mat_t *A,
  ...
  int block_row_size = 1; // need to be set according to the matrix size, matrix size must be divided by the block row size
  int block_col_size = 500; // need to be set according to the matrix size, matrix size must be divided by the block col size
  ...
  for (int A_blk_row_id = 0; A_blk_row_id < A_num_blk_per_col; A_blk_row_id ++){
    ...
    for (int B_blk_col_id = tid; B_blk_col_id < B_num_blk_per_row; B_blk_col_id += num_threads){
      
      res_row = (A_blk_row_id == A_num_blk_per_col - 1) ? A_last_blk_row : block_row_size;
      res_col = (B_blk_col_id == B_num_blk_per_row - 1) ? B_last_blk_col : block_row_size;
      ...
      for (int inside_blk_id = 0; inside_blk_id < B_num_blk_per_col; inside_blk_id ++){
        ...
        local_B_col_start = B_blk_col_id * block_row_size;
        for (int local_B_row = inside_blk_id * block_col_size; local_B_row < (inside_blk_id * block_col_size + mid_size); local_B_row ++){
          memcpy(&B_local[local_B_cpy_id * res_col], &B[local_B_row * B_Width + local_B_col_start], sizeof(B[0]) * res_col);
          local_B_cpy_id ++;
        }
        ...
        for (int A_local_row_id = 0; A_local_row_id < res_row; A_local_row_id ++){
          ...
          for (int B_local_col_id = 0; B_local_col_id < res_col; B_local_col_id ++){
            sum = Res_local[A_local_row_id * res_col + B_local_col_id];
            ...
            for (int k = sparse_mat_get_idx_ptr(A, A_row_act_id); k < sparse_mat_get_idx_ptr(A, A_row_act_id + 1); k++){
              ...
              if (col_A_id >= (inside_blk_id * block_col_size)){
                if (col_A_id < (inside_blk_id * block_col_size + mid_size)){
                  sum += sparse_mat_get_val(A, k) * B_local[(col_A_id - inside_blk_id * block_col_size) * res_col + B_local_col_id];
                }
                else {
                  break;
                }
            ...
            Res_local[A_local_row_id * res_col + B_local_col_id] = sum;
          }
        }
