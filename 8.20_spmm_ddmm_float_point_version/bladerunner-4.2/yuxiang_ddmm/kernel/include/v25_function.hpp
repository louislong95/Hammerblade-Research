...
#include <unrolling_function_outer_product.hpp>
...
template <typename TA, typename TB, typename TC>
int  __attribute__ ((noinline)) matrix_multiply_group_v25(TA *A, TB *B, TC *C, // matrix multiplication for v3, v3 implement the data distribution along column side, without scratchpad
                                uint32_t A_HEIGHT, uint32_t A_WIDTH, uint32_t B_HEIGHT, uint32_t B_WIDTH, uint32_t block_row)
{
  ...
  void (*act_ptr)(float *, float *, float *, int, int, int, int) = compute_ptr[block_size_row - 1];
        ...
        for (int B_blk_row_id = tid; B_blk_row_id < B_num_blk_per_col; B_blk_row_id += num_threads){
              ...
              for (int inside_blk_id = 0; inside_blk_id < A_num_blk_per_row; inside_blk_id ++){
                    ...
                    if ((res_row == block_size_row) && (res_col == block_size_row) && (mid_size == block_size_col)){
                          ...
                          act_ptr(tmp_result, A_blk, B_blk, res_row, mid_size, res_col, mid_size);
                    }
                    else {
