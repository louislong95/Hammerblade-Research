...
int test_smdm(int argc, char *argv[]) {
  ...
  */
  ...
  if(!strcmp("v0", test_name) || !strcmp("v1", test_name)){
    ...
    dense_matrix = dense_data_matrix; // v2 v3 v4 no tranposed
  }
  ...
  */
  ...
  if(!strcmp("v0", test_name)){
    grid_dim = { .x = 1, .y = 1};
    tg_dim = { .x = 4, .y = 4 };  // v0 perform row by row multiplication, dense matrix transposed
  }else if(!strcmp("v1", test_name)){
    grid_dim = { .x = 1, .y = 1};
    tg_dim = { .x = 4, .y = 4 };  // v1 perform blocking strategy, dense matrix transposed, block size could be flexible, and could deal with the condtion when matrix size could not be divided by the block size
  }else if(!strcmp("v2", test_name)){
    grid_dim = { .x = 1, .y = 1};
    tg_dim = { .x = 4, .y = 4 };  // v2 perform blocking strategy, dense matrix non-transposed, block size could be flexible, and could deal with the condtion when matrix size could not be divided by the block size
  }else if(!strcmp("v3", test_name)){
    grid_dim = { .x = 1, .y = 1};  
    tg_dim = { .x = 4, .y = 4 };  // v3 perform row by column outer product strategy, dense matrix non-transposed
  }else if(!strcmp("v4", test_name)){
    grid_dim = { .x = 1, .y = 1};
    tg_dim = { .x = 4, .y = 4 };  // v4 perform blocking strategy using outer product strategy 
  }
  else {
