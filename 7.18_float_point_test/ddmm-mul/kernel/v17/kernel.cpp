                                ...
                                uint32_t A_HEIGHT, uint32_t A_WIDTH, uint32_t B_WIDTH)
{
  ...
  float C_local[block_size * block_size]; // store the temporal product
  ...
  ptr_A = (tid == 0) ? sh_A11 : (tid == 1) ? sh_A12 : (tid == 2) ? sh_A21 : sh_A22;
  ptr_B = (tid == 0) ? sh_B11 : (tid == 1) ? sh_B22 : (tid == 2) ? sh_B11 : sh_B22;
  ...
  for (int A_local_row_id = 0; A_local_row_id < block_size; A_local_row_id ++) {
            for (uint32_t B_local_row_id = 0; B_local_row_id < block_size; B_local_row_id ++){
                  ...
                  for (uint32_t k = 0; k < block_size; k ++){
                        bsg_tile_group_shared_load (TA, ptr_A, (A_local_row_id * block_size + k), tmp_A);
                        bsg_tile_group_shared_load (TA, ptr_B, (B_local_row_id * block_size + k), tmp_B);
                        sum += tmp_A * tmp_B;
                  }
  ...
  ptr_A = (tid == 0) ? sh_A12 : (tid == 1) ? sh_A11 : (tid == 2) ? sh_A22 : sh_A21;
  ptr_B = (tid == 0) ? sh_B12 : (tid == 1) ? sh_B21 : (tid == 2) ? sh_B12 : sh_B21;
  
  int memback_row_idx = tid_y * block_size; // write back row index
  int memback_col_idx = tid_x * block_size; // write back column index

  for (int A_local_row_id = 0; A_local_row_id < block_size; A_local_row_id ++) {
            memback_col_idx = tid_x * block_size;
            for (uint32_t B_local_row_id = 0; B_local_row_id < block_size; B_local_row_id ++){
                  ...
                  C[memback_row_idx * B_WIDTH + memback_col_idx] = sum;
                  memback_col_idx ++;
            }
            memback_row_idx ++;
      }
